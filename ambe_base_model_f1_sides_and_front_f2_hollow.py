# ============================================
# ambe_base_model_f1_sides_and_front_f2_hollow.py
# Verified asymmetric hollow OpenMC model for the Am-Be paraffin setup
# Hollow faces: both f1/y walls and the front (+x) f2 wall.
# Solid face: the rear (-x) f2 wall.
# ============================================
...
# Explicit marker for checking that the intended module was imported.
HOLLOW_MODEL_REVISION = "F1_SIDES_AND_FRONT_F2_HOLLOW_2026-08-14"
...
DEFAULT_SOLID_CABINET_CONFIG = {
    "name": "solid_current_reference",
    "x_inner_steel_cm": 15.0,
    "x_void_cm": 0.0,
    "x_outer_steel_cm": 0.0,
    "y_inner_steel_cm": 13.0,
    "y_void_cm": 0.0,
    "y_outer_steel_cm": 0.0,
    "bottom_steel_cm": 25.0,
    "top_steel_cm": 12.0,
    "rear_x_wall_solid": True,
}

MEASURED_HOLLOW_CABINET_CONFIG = {
    "name": "f1_sides_and_front_f2_hollow_measured_nominal",
    # Both f1/y walls and the front (+x) f2 wall are hollow. From the cavity
    # outwards they contain 2 cm steel, 5 cm air, and 6 cm (f1) or 8 cm (f2)
    # outer steel. The rear (-x) f2 wall remains 15 cm solid steel.
...
def normalize_cabinet_config(cabinet_config=None):
    """Return a validated cabinet-layer specification in centimetres.

    ``None`` retains the historical solid model exactly.  Layer thicknesses
    are ordered from the cabinet cavity towards the outside.  The bottom and
    top plates remain solid because the available sketch gives no void layer
    for f3 or f4.
    """

    source = (
        DEFAULT_SOLID_CABINET_CONFIG
        if cabinet_config is None
        else cabinet_config
    )
    required = [
        "x_inner_steel_cm", "x_void_cm", "x_outer_steel_cm",
        "y_inner_steel_cm", "y_void_cm", "y_outer_steel_cm",
        "bottom_steel_cm", "top_steel_cm",
    ]
    missing = [key for key in required if key not in source]
    if missing:
        raise KeyError(f"cabinet_config is missing keys: {missing}")

    normalized = {"name": str(source.get("name", "custom_cabinet"))}
    for key in required:
    ...
    normalized["x_wall_total_cm"] = sum(
        normalized[key]
        for key in (
            "x_inner_steel_cm", "x_void_cm", "x_outer_steel_cm"
        )
    )
    normalized["y_wall_total_cm"] = sum(
        normalized[key]
        for key in (
            "y_inner_steel_cm", "y_void_cm", "y_outer_steel_cm"
        )
    )
    normalized["has_void"] = bool(
        normalized["x_void_cm"] > 0.0
        or normalized["y_void_cm"] > 0.0
    )
    normalized["rear_x_wall_solid"] = bool(
        source.get("rear_x_wall_solid", False)
    )
    normalized["hollow_faces"] = (
        ("x_max", "y_min", "y_max")
        if normalized["has_void"] and normalized["rear_x_wall_solid"]
        else ("x_min", "x_max", "y_min", "y_max")
        if normalized["has_void"]
        else ()
    )
    normalized["mode"] = (
        "three_side_hollow"
        if normalized["has_void"] and normalized["rear_x_wall_solid"]
        else "four_side_hollow"
        if normalized["has_void"]
        else "solid"
    )

    if normalized["x_wall_total_cm"] <= 0.0:
        raise ValueError("The total x-wall thickness must be positive.")
    if normalized["y_wall_total_cm"] <= 0.0:
        raise ValueError("The total y-wall thickness must be positive.")
    if normalized["bottom_steel_cm"] + normalized["top_steel_cm"] >= 96.0:
        raise ValueError("Bottom and top steel consume the complete cabinet height.")

    return normalized


def make_measured_hollow_cabinet_config(void_cm=5.0, name=None):
    """Build the three-sided hollow case with fixed wall depths.

    The nominal measured stack has a 2 cm inner steel skin and a 5 cm air
    layer. Varying ``void_cm`` changes the air depth and compensates it in the
    outer steel, so the documented 15/13 cm f2/f1 totals remain fixed. Both
    f1/y walls and the front (+x) f2 wall are hollow; the rear (-x) f2 wall is
    kept solid. The production notebook uses the fixed 5 cm configuration.
    """

    void_cm = float(void_cm)
    if not np.isfinite(void_cm) or void_cm < 0.0:
        raise ValueError("void_cm must be finite and non-negative.")

    x_total_cm = 15.0  # f2 = 2 + 5 + 8 cm
    y_total_cm = 13.0  # f1 = 2 + 5 + 6 cm
    inner_steel_cm = 2.0
    x_outer_cm = x_total_cm - inner_steel_cm - void_cm
    y_outer_cm = y_total_cm - inner_steel_cm - void_cm
    if min(x_outer_cm, y_outer_cm) < 0.0:
        raise ValueError(
            "void_cm is too large for the measured f1/f2 wall stacks."
        )

    return normalize_cabinet_config({
        "name": name or f"f1_sides_front_f2_hollow_void_{void_cm:g}_cm",
        "x_inner_steel_cm": inner_steel_cm,
        "x_void_cm": void_cm,
        "x_outer_steel_cm": x_outer_cm,
        "y_inner_steel_cm": inner_steel_cm,
        "y_void_cm": void_cm,
        "y_outer_steel_cm": y_outer_cm,
        "bottom_steel_cm": 25.0,
        "top_steel_cm": 12.0,
        "rear_x_wall_solid": True,
    })
...
def build_base_geometry_and_materials(
    ...
    cabinet_config : dict or None
        Layer specification accepted by ``normalize_cabinet_config``.  ``None``
        reproduces the historical solid-cabinet geometry without changes.
    ...
    CABINET_CONFIG = normalize_cabinet_config(cabinet_config)
    CABINET_WALL_Y = CABINET_CONFIG["y_wall_total_cm"]
    CABINET_WALL_X = CABINET_CONFIG["x_wall_total_cm"]
    CABINET_BOTTOM = CABINET_CONFIG["bottom_steel_cm"]
    CABINET_TOP = CABINET_CONFIG["top_steel_cm"]
    ...
    if 2.0 * PARAFFIN_RADIUS > min(
        CABINET_INNER_XMAX - CABINET_INNER_XMIN,
        CABINET_INNER_YMAX - CABINET_INNER_YMIN,
    ):
        raise ValueError(
            "The selected cabinet layers intersect the paraffin moderator."
        )
    ...
    if verbose:
        ...
        print(f"  Hollow cabinet faces          = {CABINET_CONFIG['hollow_faces']}")
        if CABINET_CONFIG["rear_x_wall_solid"]:
            print(
                "  Rear (-x) f2 wall              = "
                f"{CABINET_WALL_X:.1f} cm solid steel"
            )
            print(
                "  Front (+x) f2 wall stack       = "
                f"{CABINET_CONFIG['x_inner_steel_cm']:.1f} cm steel + "
                f"{CABINET_CONFIG['x_void_cm']:.1f} cm air + "
                f"{CABINET_CONFIG['x_outer_steel_cm']:.1f} cm steel"
            )
            print(
                "  Both f1/y wall stacks          = "
                f"{CABINET_CONFIG['y_inner_steel_cm']:.1f} cm steel + "
                f"{CABINET_CONFIG['y_void_cm']:.1f} cm air + "
                f"{CABINET_CONFIG['y_outer_steel_cm']:.1f} cm steel"
            )
    ...
    # The f1/f2 side-wall layers are extruded only through the free internal
    # height. The bottom (f3) and top (f4) plates are solid over the complete
    # footprint. The side layers are nested rectangular shells so their CSG
    # regions are disjoint and the plotted cross-section matches the transport
    # geometry exactly.
    side_z_region = +cav_z_min & -cav_z_max

    x_inner_outer_min = CABINET_INNER_XMIN - CABINET_CONFIG["x_inner_steel_cm"]
    x_inner_outer_max = CABINET_INNER_XMAX + CABINET_CONFIG["x_inner_steel_cm"]
    x_void_outer_min = x_inner_outer_min - CABINET_CONFIG["x_void_cm"]
    x_void_outer_max = x_inner_outer_max + CABINET_CONFIG["x_void_cm"]

    y_inner_outer_min = CABINET_INNER_YMIN - CABINET_CONFIG["y_inner_steel_cm"]
    y_inner_outer_max = CABINET_INNER_YMAX + CABINET_CONFIG["y_inner_steel_cm"]
    y_void_outer_min = y_inner_outer_min - CABINET_CONFIG["y_void_cm"]
    y_void_outer_max = y_inner_outer_max + CABINET_CONFIG["y_void_cm"]

    x_inner_min_plane = openmc.XPlane(x0=x_inner_outer_min)
    x_inner_max_plane = openmc.XPlane(x0=x_inner_outer_max)
    x_void_min_plane = openmc.XPlane(x0=x_void_outer_min)
    x_void_max_plane = openmc.XPlane(x0=x_void_outer_max)
    y_inner_min_plane = openmc.YPlane(y0=y_inner_outer_min)
    y_inner_max_plane = openmc.YPlane(y0=y_inner_outer_max)
    y_void_min_plane = openmc.YPlane(y0=y_void_outer_min)
    y_void_max_plane = openmc.YPlane(y0=y_void_outer_max)

    side_outer_footprint = (
        +cab_x_min & -cab_x_max &
        +cab_y_min & -cab_y_max
    )
    inside_outer_steel_footprint = (
        +x_void_min_plane & -x_void_max_plane &
        +y_void_min_plane & -y_void_max_plane
    )
    inside_void_footprint = (
        +x_inner_min_plane & -x_inner_max_plane &
        +y_inner_min_plane & -y_inner_max_plane
    )
    cavity_xy_footprint = (
        +cav_x_min & -cav_x_max &
        +cav_y_min & -cav_y_max
    ...
    side_inner_steel_region = (
        side_z_region &
        inside_void_footprint &
        ~cavity_xy_footprint
    )

    bottom_steel_region = cabinet_outer_region & -cav_z_min
    top_steel_region = cabinet_outer_region & +cav_z_max
    ...
    if CABINET_CONFIG["rear_x_wall_solid"]:
        # Convert every part of the nested air shell inside the rear (-x) f2
        # wall, including its two corner intersections, back to steel. The
        # remaining air region is a continuous U-shape through both f1/y
        # walls and the front (+x) f2 wall.
        cabinet_steel_region = cabinet_steel_region | rear_x_solid_region
        cabinet_void_region = side_void_region & ~rear_x_solid_region
    else:
        cabinet_void_region = side_void_region

    cabinet_wall_region = cabinet_steel_region
    ...
    if include_cabinet:
        ...
        if CABINET_CONFIG["has_void"]:
            cabinet_void_cell = openmc.Cell(
                name="Cabinet wall void",
                fill=air,
                region=cabinet_void_region,
            )
        else:
            cabinet_void_cell = None
    ...
    if include_cabinet:
        ...
        if cabinet_void_cell is not None:
            root_cells.insert(0, cabinet_void_cell)
    ...
    cabinet_envelope_volume_cm3 = cabinet_outer_volume_cm3 - cabinet_inner_volume_cm3

    free_internal_height_cm = CABINET_INNER_ZMAX - CABINET_INNER_ZMIN
    inside_outer_steel_x_cm = (
        CABINET_X - 2.0 * CABINET_CONFIG["x_outer_steel_cm"]
    )
    inside_outer_steel_y_cm = (
        CABINET_Y - 2.0 * CABINET_CONFIG["y_outer_steel_cm"]
    )
    inside_void_x_cm = (
        inside_outer_steel_x_cm - 2.0 * CABINET_CONFIG["x_void_cm"]
    )
    inside_void_y_cm = (
        inside_outer_steel_y_cm - 2.0 * CABINET_CONFIG["y_void_cm"]
    )
    four_side_void_cross_section_cm2 = (
        inside_outer_steel_x_cm * inside_outer_steel_y_cm
        - inside_void_x_cm * inside_void_y_cm
    )

    rear_replaced_void_cross_section_cm2 = 0.0
    if CABINET_CONFIG["rear_x_wall_solid"]:
        rear_replaced_void_cross_section_cm2 = (
            CABINET_CONFIG["x_void_cm"] * inside_outer_steel_y_cm
            + CABINET_CONFIG["x_inner_steel_cm"]
            * 2.0 * CABINET_CONFIG["y_void_cm"]
        )
    ...
    if cabinet_void_cross_section_cm2 < 0.0:
        raise RuntimeError("Calculated cabinet void area is negative.")

    cabinet_void_volume_cm3 = (
        cabinet_void_cross_section_cm2 * free_internal_height_cm
    )
    cabinet_wall_volume_cm3 = cabinet_envelope_volume_cm3 - cabinet_void_volume_cm3
    ...
    if verbose:
        ...
        print(f"  Cabinet void volume:         {cabinet_void_volume_cm3:.1f} cm^3")
